        ...
        return (
            <div className="flex flex-col bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden"
                ...
                <div className="px-5 py-2.5 bg-gray-50 border-b border-gray-100 flex items-center gap-4 flex-wrap shrink-0">
                    {[{ icon: Ticket, val: activeComplaint.ticketId }, { icon: Hash,   val: `Seat ${activeComplaint.seatNumber}` }, { icon: MapPin, val: activeComplaint.route }, { icon: User,   val: activeComplaint.passengerName }, { label: 'Travel', val: activeComplaint.travelDate }].map(({ icon: Icon, label, val }, i) => (
                                    ...
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-start justify-between gap-3 mb-1.5">
                                                <p className="font-bold text-gray-900 text-sm leading-tight">Passenger: {c.passengerName}</p>
                                                <StatusBadge status={c.status} />
                                            </div>
                                            <div className="flex items-center gap-2 mb-2 flex-wrap">
                                                <span className="flex items-center gap-1 text-[10px] text-gray-400 font-medium"><Bus size={9} className="text-gray-400" />{c.busName}</span>
                                                <span className="text-gray-200">•</span>
                                                <span className="flex items-center gap-1 text-[10px] text-gray-400 font-medium"><MapPin size={9} className="text-gray-400" />{c.route}</span>
                                                <span className="text-gray-200">•</span>
                                                <span className="flex items-center gap-1 text-[10px] text-gray-400 font-medium"><Ticket size={9} className="text-gray-400" />{c.ticketId}</span>
                                                <span className="text-gray-200">•</span>
                                                <span className="flex items-center gap-1 text-[10px] text-gray-400 font-medium"><Hash size={9} className="text-gray-400" />Seat {c.seatNumber}</span>
                                            </div>
                                            <p className="text-xs text-gray-500 truncate leading-relaxed">
                                                <span className={cn('font-semibold mr-1', lastMsg.sender === 'operator' ? 'text-blue-500' : 'text-gray-600')}>
                        ...
                        {filteredLegacy.map(d => (
                            ...
                            >
                                <div className="flex items-start gap-4">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-start justify-between gap-3 mb-1.5">
                                            <p className="font-bold text-gray-900 text-sm leading-tight">Passenger: {d.user.name}</p>
                                            <StatusBadge status={d.status} />
                                        </div>
                                        <div className="flex items-center gap-2 mb-2 flex-wrap">
                                            <span className="flex items-center gap-1 text-[10px] text-gray-400 font-medium"><MapPin size={9} className="text-gray-400" />{d.route}</span>
                                            <span className="text-gray-200">•</span>
                                            <span className="flex items-center gap-1 text-[10px] text-gray-400 font-medium"><Calendar size={9} className="text-gray-400" />{d.date}</span>
                                            <span className="text-gray-200">•</span>
                                            <PriorityBadge priority={d.priority} />
                                        </div>
                                        <p className="text-xs text-gray-500 leading-relaxed font-semibold">Issue: <span className="font-normal text-gray-500">{d.issue}</span></p>
                                        <p className="text-[10px] text-gray-400 mt-2 font-mono">{d.id}</p>
                                    </div>
                                </div>
                            </Card>
                        ))}
