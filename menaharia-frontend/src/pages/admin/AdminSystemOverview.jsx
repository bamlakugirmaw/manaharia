...
export default function AdminSystemOverview() {
    ...
    // GET /v1/operators — for top operators list
    const { data: operatorsResponse } = useOperators({ limit: 3, status: 'ACTIVE' });
    const topOperators = (() => {
        const raw = Array.isArray(operatorsResponse)
            ? operatorsResponse
            : (operatorsResponse?.data ?? []);
        return raw.slice(0, 3).map((op, i) => ({
            id:     op.id ?? i,
            name:   op.name ?? op.companyName ?? '—',
            revenue: op.revenue ? `ETB ${Number(op.revenue).toLocaleString()}` : '—',
            growth: op.revenueGrowth ?? '+0%',
            logo:   op.logo ?? `/images/Enhanced_Bus_Images/Selam_Bus1.jpg`,
        }));
    })();

    // Revenue chart — backend may return monthlyRevenue array
    const revenueData = dashData?.monthlyRevenue ?? dashData?.revenueChart ?? FALLBACK_REVENUE;

    const kpis = [
        {
            title: 'Total Revenue',
            value: dashLoading ? '—' : (dashData?.totalRevenue ? `${Number(dashData.totalRevenue / 1_000_000).toFixed(1)}M` : '—'),
            unit: 'ETB',
            icon: TrendingUp,
            color: 'bg-green-500',
            bg: 'bg-green-50',
        },
        {
            ...
            value: dashLoading ? '—' : (dashData?.totalBookings?.toLocaleString() ?? '—'),
            icon: BookOpen,
            color: 'bg-blue-500',
            bg: 'bg-blue-50',
        },
        {
            ...
            value: dashLoading ? '—' : (dashData?.activeOperators ?? '—'),
            icon: Building,
            color: 'bg-purple-500',
            bg: 'bg-purple-50',
        },
        {
            title: 'Active Users',
            value: dashLoading ? '—' : (dashData?.activeUsers?.toLocaleString() ?? '—'),
            icon: Users,
            color: 'bg-orange-500',
            bg: 'bg-orange-50',
        },
        {
            title: 'Avg Occupancy',
            value: dashLoading ? '—' : (dashData?.avgOccupancy ? `${dashData.avgOccupancy}%` : '—'),
            icon: Shield,
            color: 'bg-teal-500',
            bg: 'bg-teal-50',
        },
                ...
                <div className="h-[350px] w-full mt-4">
                    <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={revenueData} margin={{ top: 10, right: 10, left: 0, bottom: 30 }}>
                            ...
                            />
                            <Area type="monotone" dataKey="revenue" stroke="#3B82F6" strokeWidth={4} fillOpacity={1} fill="url(#areaGradient)" />
                        </AreaChart>
                    </ResponsiveContainer>
                </div>
            </Card>
                        ...
                        <div key={op.id} className="flex justify-between items-center pt-6 border-t border-gray-50 first:border-0 first:pt-0 group hover:bg-gray-50/50 p-3 rounded-xl transition-colors">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-green-500 font-bold mt-0.5 uppercase tracking-widest">{op.growth} this month</p>
                                </div>
                            </div>
